...
# Grant view access to the demo app namespace
...
Navigate to your RHDH URL and sign in with GitHub. Verify the `demo-app` component shows:
...
oc logs -n rhdh deployment/backstage-rhdh-instance -c backstage-backend -f | \
  grep -i "demo-app\|catalog.*refresh"
```
...
Disable the local argocd plugin — only use the OCI versions:
...
The ArgoCD frontend plugin requires the Kubernetes plugin. Enable both `backstage-plugin-kubernetes-backend-dynamic` and `backstage-plugin-kubernetes`.
...
Check the `catalog-info.yaml` annotations — do not use `backstage.io/kubernetes-label-selector` unless the pods actually have that label. Use only:
...
Also ensure the pod template in `deployment.yaml` has the `backstage.io/kubernetes-id` label, not just the deployment metadata.

### Kubernetes tab shows TLS error (`self-signed certificate in certificate chain`)

The `skipTLSVerify` flag is not honored in all code paths. The fix is to set `NODE_EXTRA_CA_CERTS` in the Backstage CR pointing to the mounted service account CA, and enable `automountServiceAccountToken: true` with the `rhdh-kubernetes` service account:
...
The `view` ClusterRole does not include OpenShift Route resources. Create a dedicated role:
...
In RHDH 1.9 community ArgoCD plugins moved to OCI. Use:
...
### Route URL mismatch after deployment

Verify the actual route hostname:

```bash
oc get route -n rhdh
```

The route uses `edge/Redirect` TLS — always use `https://`.

### Catalog entity takes a long time to update

Force an immediate refresh by clicking the three-dot menu on the entity page in RHDH and selecting **Schedule Entity Refresh**. Watch for the refresh in logs:

```bash
oc logs -n rhdh deployment/backstage-rhdh-instance -c backstage-backend -f | \
  grep -i "demo-app\|Refreshed entity"
```
...
- `customResources` apiVersion should be just `v1` not the full group/version — use `group: 'route.openshift.io'` and `apiVersion: 'v1'`
